...
export function AlbumDetailView() {
  ...
  if (!album) return <EmptyState title="Album not found." hint="It may have been removed from your library." />
    ...
    <div className="mx-auto max-w-[1000px]">
      ...
      >
          ...
          <div className="mt-5 flex flex-col gap-2.5">
            ...
            <div className="flex gap-2.5">
              <GlassButton className="flex-1" onClick={() => {
                ...
                player.playSong(ids[Math.floor(Math.random() * ids.length)], ids, 'Shuffle')
                if (!player.shuffle) player.toggleShuffle()
              }}>
                <span className="flex items-center justify-center gap-2"><IconShuffle size={14} /> Shuffle</span>
              </GlassButton>
              <button
                className="icon-btn glass-soft h-[42px] w-[42px] rounded-full"
                onClick={() => lib.toggleFavAlbum(album.id)}
                aria-label={fav ? 'Remove from favorites' : 'Add album to favorites'}
              ...
              <button
                ...
                aria-label="Customize album layout"
              >
        ...
        <div className="min-w-0">
          <div className="text-[10.5px] uppercase tracking-[0.3em]" style={{ color: 'var(--c-ink-faint)' }}>Album</div>
          ...
          <p className="mt-1 text-[13.5px]" style={{ color: 'var(--c-ink-dim)' }}>
            ...
            {album.year ? ` · ${album.year}` : ''}{album.genre ? ` · ${album.genre}` : ''} · {songs.length} tracks · {fmtTime(duration)}
          </p>
...
function LayoutEditor({ albumId }: { albumId: string }) {
  ...
  const rows: Array<{ key: keyof AlbumLayout; label: string; min: number; max: number; step: number; unit?: string }> = [
    ...
    { key: 'rx', label: 'Rotate X', min: -45, max: 45, step: 1, unit: '°' },
    { key: 'ry', label: 'Rotate Y', min: -45, max: 45, step: 1, unit: '°' },
    { key: 'rz', label: 'Rotate Z', min: -45, max: 45, step: 1, unit: '°' },
    { key: 'scale', label: 'Scale', min: 0.5, max: 1.6, step: 0.01 },
    { key: 'shadow', label: 'Shadow', min: 0, max: 1.5, step: 0.05 },
    { key: 'depth', label: 'Depth', min: 0, max: 120, step: 1 },
    { key: 'reflection', label: 'Reflection', min: 0, max: 1, step: 0.05 },
  ]
